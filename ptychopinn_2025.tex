...
Table~\ref{tab:sim_lines_metrics} reveals a clear interaction between probe structure and overlap. With the idealized (flat-phase) probe, removing overlap ($C_g=1$) drops phase SSIM from 0.996 to 0.410; with the experimental (curved) probe, the same change yields 0.966 to 0.886. Probe curvature largely compensates for the loss of overlap-based redundancy, confirming that structured phase diversity is the operative mechanism for single-shot reconstruction. Under overlap ($C_g=4$), the idealized probe slightly outperforms the experimental probe on phase (0.996 vs 0.966), suggesting that the experimental probe's spatial complexity introduces reconstruction ambiguities that overlap alone does not fully resolve. These trends indicate that overlap and probe diversity are partially substitutable sources of constraint, but their interaction is non-trivial and warrants further study across a broader range of probe geometries.
