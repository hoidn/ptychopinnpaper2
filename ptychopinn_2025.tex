...
\author{O. Hoidn,\authormark{1,*} A. Mishra,\authormark{1} S. Henke,\authormark{1} A. Vong,\authormark{1} and M. Seaberg\authormark{1}}
...
\begin{abstract*}
Single-shot coherent imaging carries benefits in synchrotron and XFEL-based X-ray microscopy because it lowers photon dose, removes the overhead of densely overlapping scans, and increases imaging throughput. We extend PtychoPINN~\cite{Hoidn2023PtychoPINN}, a physics-constrained neural network for self-supervised ptychographic reconstruction, to deliver \emph{overlap-free, single-shot} reconstructions in a Fresnel (near-field) CDI geometry and also accelerate conventional multi-shot ptychography. The framework learns directly from diffraction intensities using a differentiable forward model of coherent scattering and a Poisson photon-counting likelihood. It optionally enforces real-space constraints through coordinate-based grouping, which enables multi-shot reconstruction under arbitrary scan geometries. Per-GPU throughput is approximately $10\times$ that of high-performance conventional solvers at matched resolution. Reconstructions remain accurate at low counts ($\sim\!10^4$ photons/frame) and zero real-space overlap. Compared to a supervised baseline with the same network architecture, our framework achieves comparable reconstruction quality with as few as 1024 training samples and generalizes across beamlines without retraining. In synthetic overlap ablations with an experimental probe, overlap-free \emph{single-shot} reconstruction reaches amplitude SSIM 0.904 versus 0.968 for overlap-constrained reconstruction ($\Delta=0.064$). This unification of single-exposure Fresnel CDI and overlapped ptychography within a single framework supports dose-efficient imaging at XFELs at substantially higher throughput than previously feasible.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        ...
        \caption{Representative reconstructions across dose.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
...
PtychoPINN processes approximately 6.1k diffraction patterns per second at $64\times64$ image resolution and 2.6k patterns per second at $128\times128$ in single-GPU inference measurements, excluding stitching/reassembly time. The upper limit of computational performance in conventional reconstruction is SHARP, whose authors report approximately 300 frames per second per GPU at $128\times128$ ~\cite{Marchesini2016SHARP}. PtychoPINN therefore has approximately a 9-fold advantage in reconstruction throughput compared to high-performance iterative solvers. (The most commonly-used conventional solvers under practical settings, such as least squares maximum likelihood (LSQ-ML) and rPIE, are notably slower than SHARP.)
...
The main limitation is the fixed-probe assumption: our formulation requires a pre-estimated probe and does not model probe drift or position errors during training. Joint probe retrieval within the self-supervised loop and differentiable position refinement are natural extensions that would broaden the range of experimental conditions the framework can handle. On the computational side, mixed-precision execution and kernel fusion should yield roughly a $2\times$ throughput improvement with no algorithmic changes. The more consequential bottleneck is the CNN backbone: convolutional layers lack global receptive fields, which limits both high-spatial-frequency accuracy at a given $N$ and scaling to larger patch dimensions. Architectures with spectral features (e.g., Fourier neural operators) provide global mixing in every layer, addressing both limitations. Because the framework is modular---backbone, forward model, and loss are independent---these extensions and adaptations to other geometries (e.g., Bragg CDI) require changes to individual components rather than to the overall formulation.

In summary, coupling a Poisson photon-counting likelihood with a differentiable forward model yields dose efficiency, robustness to sparse or irregular acquisition, and support for single-shot Fresnel CDI, unifying overlapped ptychography and single-exposure imaging within one framework.
