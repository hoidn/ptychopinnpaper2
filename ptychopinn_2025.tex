...
\begin{abstract}
Single-shot coherent imaging is important for XFEL science because it lowers photon dose, removes the overhead of overlapping scans, and enables real-time feedback. We extend PtychoPINN~\cite{Hoidn2023PtychoPINN}, a physics-constrained neural network for self-supervised ptychographic reconstruction, to deliver \emph{overlap-free, single-shot} reconstructions in a Fresnel (near-field) CDI geometry and also accelerate conventional multi-shot ptychography. The framework learns directly from diffraction intensities using a differentiable forward model of coherent scattering an a Poisson photon-counting likelihood. It optionally enforces real-space constraints through coordinate-based grouping, which enables multi-shot reconstruction under arbitrary scan geometries. Per-GPU throughput is approximately $9\times$ that of high-performance conventional solvers at matched resolution. Reconstructions remain accurate at low counts ($\sim\!10^4$ photons/frame) and zero real-space overlap. Compared to a supervised baseline with the same network architecture, PtychoPINN achieves comparable reconstruction quality with as few as 1024 training samples and generalizes across beamlines without retraining. In the Fresnel regime we obtain \emph{single-shot} reconstructions with little loss of quality compared to overlap-constrained reconstruction. This unification of single-exposure Fresnel CDI and overlapped ptychography within a single framework supports dose-efficient, real-time imaging at XFELs at substantially higher throughput than previously feasible.
