...
\begin{abstract*}
Single-shot coherent imaging carries benefits in synchrotron and X-ray Free-Electron Laser (XFEL)-based X-ray microscopy because it lowers photon dose, removes the overhead of densely overlapping scans, and increases imaging throughput. We extend PtychoPINN (O. Hoidn \emph{et al.}, \emph{Scientific Reports} \textbf{13}, 22789, 2023), a physics-constrained neural network for self-supervised ptychographic reconstruction, to deliver \emph{overlap-free, single-shot} reconstructions in a Fresnel (near-field) coherent diffraction imaging (CDI) geometry and also accelerate conventional multi-shot ptychography. The framework learns directly from diffraction intensities using a differentiable forward model of coherent scattering and a Poisson photon-counting likelihood. It optionally enforces real-space constraints through coordinate-based grouping, which enables multi-shot reconstruction under arbitrary scan geometries. At matched $128\times128$ resolution, throughput per graphics processing unit (GPU) is approximately $40\times$ that of a high-performance least-squares maximum-likelihood (LSQ-ML) baseline, assuming 100 iterations for convergence. Reconstructions remain accurate at low counts ($\sim\!10^4$ photons/frame) and zero real-space overlap. Against a data-saturated supervised model with the same backbone trained on 16,384 images, PtychoPINN reaches comparable quality with only 1,024 diffraction images and generalizes to illumination profiles unseen during training. We quantitatively evaluate on synthetic data and validate on experimental data from the Advanced Photon Source and the Linac Coherent Light Source. In ablations, overlap-free \emph{single-shot} reconstruction reaches amplitude structural similarity index measure (SSIM) 0.904 versus 0.968 for overlap-constrained reconstruction ($\Delta=0.064$). This unification of single-exposure Fresnel CDI and overlapped ptychography within a single framework supports dose-efficient imaging at XFELs at substantially higher throughput than previously feasible.
