...
Finally, the architecture that we have presented is modular in that the inverse-mapping neural network backbone, physics constraints, and loss can be modified independently. For example, extending the approach to Bragg CDI and reflection geometries would primarily require forward-model adaptations, with no changes to the neural network or training procedure. Of more immediate interest, we intend to replace the CNN backbone with a fourier neural operator-inspired network. This improvement, which we believe will close the gap in resolution between existing ML procedures and conventional reconstruction, will be the focus of our upcoming work. 
