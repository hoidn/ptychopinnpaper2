...
The self-supervised framework provides three practical advantages. First, the forward-model constraint improves data efficiency: PtychoPINN reaches a given phase SSIM with roughly an order of magnitude fewer training samples than the supervised baseline (Fig.~\ref{fig:ssim}). Second, the Poisson NLL loss correctly weights low-count detector pixels where high-$q$ signal is weak and noise-dominated, improving dose efficiency relative to MAE (Fig.~\ref{fig:dose}). Third, self-supervision in diffraction space avoids overfitting nuisance parameters, such as global phase offsets, that are intrinsic to the coherent imaging inverse problem. Real-space supervision is susceptible to these degeneracies, which likely explains the supervised baseline's poor transferability under the APS$\!\to\!$LCLS distribution shift (Fig.~\ref{fig:fivepanel}). These capabilities are particularly relevant for dynamic or radiation-sensitive samples at high-rate sources, where overlap requirements, labeled training data, and photon budget are simultaneous constraints.
