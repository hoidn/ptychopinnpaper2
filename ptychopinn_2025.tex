...
Modern light sources, such as fourth-generation synchrotrons and X-ray Free-Electron Lasers (XFELs), generate coherent diffraction data at very high rates (facility- and detector-dependent, often terabytes per hour to higher regimes)~\cite{LCLSIIHE_DesignPerf}. This data deluge poses a significant challenge for Coherent Diffraction Imaging (CDI) techniques like ptychography, as traditional image reconstruction relies on computationally intensive iterative phase retrieval algorithms. An analytical bottleneck has emerged where offline processing time vastly exceeds data acquisition time. This disparity not only delays scientific insights but, more critically, precludes real-time feedback and on-the-fly experimental steering---capabilities essential for maximizing the efficiency and discovery potential of these facilities. Consequently, there is a pressing need for new reconstruction paradigms that deliver high-fidelity results at accelerated rates without sacrificing reliability or robustness.
...
Figure~\ref{fig:smalldat} compares reconstructions on APS Siemens-star data across two sampling budgets (512 and 8192 diffraction patterns). The physics-informed model preserves clear edge structure and contrast in both regimes, while the supervised matched-backbone model (PtychoNN-style) degrades visibly at lower data availability. On the synthetic Siemens-star dataset (simulated from APS Siemens-star reconstructions), the physics-informed model also attains higher phase fidelity than the supervised matched-backbone model; see Table~\ref{tab:results_16384}.
...
\begin{figure}[htbp]
    ...
    \caption{Structural similarity of PtychoPINN, the supervised matched-backbone model, and a conventional iterative reference (rPIE in Tike) as a function of training set size.}
    \label{fig:ssim}
\end{figure}

\subsection{Out-of-distribution Generalization}

Figure~\ref{fig:fivepanel} includes an in-distribution LCLS control (train LCLS XPP, test LCLS XPP) and an out-of-distribution transfer setting (train APS, test LCLS XPP). Under APS$\!\to\!$LCLS shift, the supervised baseline degrades markedly, whereas the physics-informed model preserves edge structure despite phase distortion and reduced quantitative fidelity. The transfer evidence is qualitative but still informative: under the same APS$\!\to\!$LCLS transfer setup, the supervised baseline largely collapses while the physics-informed model retains interpretable structure. The reference column in Fig.~\ref{fig:fivepanel} is an ePIE reconstruction of the LCLS data.
...
% =======================
% Bottom row (supervised matched-backbone)
% =======================
...
PtychoPINN processes approximately 4.6k diffraction patterns per second for $64\times64$ patches on an NVIDIA RTX 3090, excluding stitching/reassembly time. Marchesini et al. report SHARP reconstruction of 10,000 frames at $128\times128$ in under 2 s on 16 GPUs for a simulation benchmark~\cite{Marchesini2016SHARP}, i.e., $>$5000 frames/s aggregate. Assuming linear per-GPU normalization from that multi-GPU result gives $>$312.5 frames/s per GPU at $128\times128$; rescaling from $128\times128$ to $64\times64$ under an $N^2$ computational-scaling assumption gives $>$1250 frames/s per GPU and an estimated per-GPU throughput ratio of approximately $3.7\times$ in favor of PtychoPINN. This ratio is an approximate normalized reference, not a direct matched-condition single-GPU benchmark.
...
We assume a fixed, pre-estimated probe during both training and inference; joint probe retrieval within the self-supervised loop is a natural extension. Probe drift and position errors are not modeled explicitly and could be addressed by stochastic position models or differentiable refinement. Extending the approach to Bragg CDI and reflection geometries would primarily require forward-model and sampling adaptations. The architecture is modular: the inverse-network backbone, physics operators, and loss can be improved independently. Additional speedups are expected from mixed-precision execution and backbone-level architectural optimization. At fixed wall-time, these gains can be used for better reconstruction quality or higher target resolution. Replacing the CNN backbone with an FNO backbone is a plausible path to better scaling at larger $N$ and improved high-resolution reconstruction quality.
...
\texttt{C\_g} (code default) & 4 & Number of patterns per group \\
\texttt{C\_g} (APS/LCLS runs) & 1 & Single-shot, overlap-free setting used in reported APS/LCLS experiments \\
