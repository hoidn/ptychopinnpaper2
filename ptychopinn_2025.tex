...
\subsection*{Single-shot regime and probe requirements}
...
\subsection*{Benefits of self-supervised training}
The self-supervised framework provides three practical advantages. First, the forward-model constraint improves data efficiency: PtychoPINN reaches a given phase SSIM with roughly an order of magnitude fewer training samples than the supervised baseline (Fig.~\ref{fig:ssim}). Second, the Poisson NLL loss correctly weights low-count detector pixels where high-$q$ signal is weak and noise-dominated, improving dose efficiency relative to MAE (Fig.~\ref{fig:dose}). Third, self-supervision in diffraction space avoids overfitting nuisance parameters, such as global phase offsets, that are intrinsic to the coherent imaging inverse problem. Real-space supervision is susceptible to these degeneracies, which likely explains both the supervised baseline's train--test gap on held-out scan positions (Fig.~\ref{fig:smalldat}) and its collapse under cross-facility transfer (Fig.~\ref{fig:fivepanel}). These capabilities are particularly relevant for dynamic or radiation-sensitive samples at high-rate sources, where overlap requirements, labeled training data, and photon budget are simultaneous constraints.

\subsection*{Limitations and outlook}
The main limitation is the fixed-probe assumption: our formulation requires a pre-estimated probe and does not model probe drift or position errors during training. Joint probe retrieval within the self-supervised loop and differentiable position refinement are natural extensions that would broaden the range of experimental conditions the framework can handle. On the computational side, mixed-precision execution and kernel fusion should yield roughly a $2\times$ throughput improvement with no algorithmic changes. The more consequential bottleneck is the CNN backbone's limited resolution ($N \leq 128$); replacing it with a Fourier neural operator-inspired architecture is the most direct path to closing the resolution gap with conventional solvers. Because the framework is modular---backbone, forward model, and loss are independent---these extensions and adaptations to other geometries (e.g., Bragg CDI) require changes to individual components rather than to the overall formulation.
